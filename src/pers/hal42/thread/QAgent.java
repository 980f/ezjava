...
///////////////////////////
  ...
  public void run(){//implements Runnable
    ...
    try(Finally pop = dbg.Push(myname)) {
      ...
      if(finishing) {
        while(!fifo.isEmpty()) {
          Qtype todo = fifo.next();
          dbg.VERBOSE(format("{0} finally runone", myname));
          actor.runone(todo);
        }
      }
  ...
  public synchronized void Start(){
    if(amStopped) {
      ...
      try {
        ...
        dbg.WARNING(MessageFormat.format("QAgent.Start() ignored:{0}", itse));
      }
    } else {
  ...
  /**
   * NB: this can hang if processing of an item hangs.
   */
  public void Finish(){
    finishing = true;
    killed = true;
    ThreadX.join(thread, 0);
  }
