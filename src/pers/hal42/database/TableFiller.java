...
import pers.hal42.lang.ReflectX.AnnotationWalker;
...
public class TableFiller {
  ...
  /**
   * @returns a single row inserter with '?'s for data.
   */
  public static QueryString makeInsertFor(Class hasDbColumns, TableInfo ti) {
    QueryString.Lister inserter = QueryString.Insert(ti, false);

    final AnnotationWalker fields = new AnnotationWalker(hasDbColumns, DbColumn.class);

    fields.forEachRemaining(field -> {
      inserter.prepareSet(field.getName());
    });
  ...
  public static <T> void addToTable(T thing, TableInfo ti, DBMacros jdbc) {
    ...
    final QueryString inserter = makeInsertFor(ref, ti);
    final AnnotationWalker fields = new AnnotationWalker(ref, DbColumn.class);
    ...
    try (PreparedStatement ps = jdbc.makePreparedStatement(inserter.toString()); PreparedStatementInserter psi = new PreparedStatementInserter(ps)) {
      fields.forEachRemaining(field -> {
        try {
          field.setAccessible(true);//be aggressive, but only useful if we are doing local walking. //todo:1 have agressive bit on walker
          String name = field.getName();
